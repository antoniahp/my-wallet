...
class DepositMoneyCommandHandler:
    ...
    def handle(self, command:DepositAmountCommand):
        with transaction.atomic():
            ...
            email = EmailMessage(
                "Your last movement",
                f"""<b>{account_filtered.user.name} {account_filtered.user.surname}</b> you have deposit {command.deposit_amount} of account {account_filtered.account_number} with concept: {command.concept}.
